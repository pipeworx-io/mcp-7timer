...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, '7Timer!');
}
...
function coords(args: Record<string, unknown>): { lat: number; lon: number } {
  ...
  if (!Number.isFinite(lat) || !Number.isFinite(lon)) {
    // THROW rather than return an error shape. The `user_error:` prefix is a
    // routing token for the gateway's classifier, and stripClassPrefix only
    // runs on the THROWN path — a returned {error} object is passed through as
    // data, so the token reached the caller verbatim ("user_error: provide
    // numeric latitude and longitude"), which reads as an internal leaking.
    // Throwing also gets the caller the standard envelope with a retry_hint.
    throw new Error('user_error: provide numeric latitude and longitude');
  }
  return { lat, lon };
}
